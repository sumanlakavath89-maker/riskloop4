      ...
      for (const item of cfg.officialDates) {
        if (item.date >= fromDate && item.date <= toDate) {
          ...
          events.push({
            ...
            previous: (item.previous !== undefined && item.previous !== null && item.previous !== '' && item.previous !== '—') ? String(item.previous) : null,
            forecast: (item.forecast !== undefined && item.forecast !== null && item.forecast !== '' && item.forecast !== '—') ? String(item.forecast) : null,
            actual: (item.actual !== undefined && item.actual !== null && item.actual !== '' && item.actual !== '—') ? String(item.actual) : null,
            unit: cfg.unit || '%',
            source: cfg.source,
